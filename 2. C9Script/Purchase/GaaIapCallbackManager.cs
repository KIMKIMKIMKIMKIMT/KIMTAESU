    ...
    public void PurchaseUpdatedListener(string result)
    {
        try
        {
            ...
            if (iapResult.IsSuccess())
            {
                ...
                {
                    PurchaseUpdatedSuccessEvent(null, null, 0, 0);
                }
            }
            else
            {
        ...
        try
        {
            ...
            if (iapResult.IsSuccess())
            {
                if (purchaseRes.totalCount > 0)
                {
                    ...
                    int count = purchaseRes.count;
                    int totalCount = purchaseRes.totalCount;
                    QueryPurchasesSuccessEvent(purchaseData, signature, count, totalCount);
                }
                else
                {
                    QueryPurchasesSuccessEvent(null, null, 0, 0);
                }
            }
            else
            {
                QueryPurchasesErrorEvent(iapResult);
            }
        }
        catch (System.Exception ex)
        {
            ...
            QueryPurchasesErrorEvent(errorResult);
        }
    }
